...
void usage(void) {
    printf("(c)2015 befinitiv. Based on packetspammer by Andy Green.  Licensed under GPL2\n"
           "\n"
...
void open_and_configure_interface(const char* name, int port, monitor_interface_t* interface) {
    ...
    default: fprintf(stderr, "!!! unknown encapsulation on %s !\n", name); exit(1);
    }
...
void process_payload(uint8_t* data, size_t data_len, int crc_correct, block_buffer_t* block_buffer_list, int adapter_no) {
    ...
    block_num = wph->sequence_number / (param_data_packets_per_block + param_fec_packets_per_block); // if
    // aram_data_packets_per_block+param_fec_packets_per_block
    // would be limited to powers of two, this could be replaced
    ...
    // or we have received a block_num that is several times smaller than the current window of buffers -> this indicated that
    // either the window is too small or that the transmitter has been restarted
    ...
    if ((block_num > max_block_num || tx_restart) && crc_correct) {
        ...
        if (last_block_num != -1) {
            ...
            // we have both pointers to the packet buffers (to get information about crc and vadility) and raw data pointers for
            // fec_decode
            ...
            while (di < param_data_packets_per_block || fi < param_fec_packets_per_block) {
                if (di < param_data_packets_per_block) {
                    ...
                    if (data_pkgs[di]->valid && !data_pkgs[di]->crc_correct) datas_corrupt++;
                    di++;
                }
                ...
                if (fi < param_fec_packets_per_block) {
                    ...
                    if (fec_pkgs[fi]->valid && !fec_pkgs[fi]->crc_correct) fecs_corrupt++;
            ...
            while (di < param_data_packets_per_block && fi < param_fec_packets_per_block) {
                ...
                // if this FEC is corrupted and there are more lost packages than good fecs we should replace this DATA even with
                // this corrupted FEC
                ...
                if (fec_pkgs[fi]->crc_correct) good_fecs--;
            ...
            if (reconstruction_failed) {
                ...
                fprintf(stderr, "Could not fully reconstruct block %x! Damage rate: %f (%d / %d blocks)\n", last_block_num,
                        1.0 * rx_status->damaged_block_cnt / rx_status->received_block_cnt, rx_status->damaged_block_cnt,
                        rx_status->received_block_cnt);
                debug_print("Data mis: %d\tData corr: %d\tFEC mis: %d\tFEC corr: %d\n", datas_missing_c, datas_corrupt_c,
                            fecs_missing_c, fecs_corrupt_c);
            }

            // decode data and write it to STDOUT
            fec_decode((unsigned int)param_packet_length, data_blocks, param_data_packets_per_block, fec_blocks, fec_block_nos,
    ...
    for (i = 0; i < param_block_buffers; ++i) {
        if (rbb->block_num == block_num) { break; }
        rbb++;
    }
    ...
    if (i != param_block_buffers) {
        ...
        packet_num = wph->sequence_number % (param_data_packets_per_block + param_fec_packets_per_block); // if retr_block_size
                                                                                                          // would be limited to
                                                                                                          // powers of two, this
                                                                                                          // could be replace by a
...
void process_packet(monitor_interface_t* interface, block_buffer_t* block_buffer_list, int adapter_no) {
    ...
    if (ieee80211_radiotap_iterator_init(&rti, (struct ieee80211_radiotap_header*)pu8Payload, ppcapPacketHeader->len) < 0) return;

    while ((n = ieee80211_radiotap_iterator_next(&rti)) == 0) {

        switch (rti.this_arg_index) {
        case IEEE80211_RADIOTAP_RATE: prd.m_nRate = (*rti.this_arg); break;
        ...
        case IEEE80211_RADIOTAP_ANTENNA: prd.m_nAntenna = (*rti.this_arg) + 1; break;

        case IEEE80211_RADIOTAP_FLAGS: prd.m_nRadiotapFlags = *rti.this_arg; break;

        case IEEE80211_RADIOTAP_DBM_ANTSIGNAL: rx_status->adapter[adapter_no].current_signal_dbm = (int8_t)(*rti.this_arg); break;
        }
    }
    ...
    if (!checksum_correct) rx_status->adapter[adapter_no].wrong_crc_cnt++;
...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        case 'b': param_data_packets_per_block = atoi(optarg); break;

        case 'r': param_fec_packets_per_block = atoi(optarg); break;

        case 'd': param_block_buffers = atoi(optarg); break;
    ...
    for (i = 0; i < param_block_buffers; ++i) {
        ...
        block_buffer_list[i].packet_buffer_list =
            lib_alloc_packet_buffer_list(param_data_packets_per_block + param_fec_packets_per_block, MAX_PACKET_LENGTH);
    }
    ...
    for (;;) {
        ...
        for (i = 0; i < num_interfaces; ++i) {
            ...
            if (FD_ISSET(interfaces[i].selectable_fd, &readset)) { process_packet(interfaces + i, block_buffer_list, i); }
        }
    }
