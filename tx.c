...
void usage(void) {
    printf("(c)2015 befinitiv. Based on packetspammer by Andy Green.  Licensed under GPL2\n"
           ...
           "-s <stream> If <stream> is > 1 then the parameter changes \"tx\" input from stdin to named fifos. Each fifo "
           "transports a stream over a different port (starting at -p port and incrementing). Fifo names are \"%s\". (default "
           "1)\n"
           "Example:\n"
           "  iwconfig wlan0 down\n"
...
void set_port_no(uint8_t* pu, uint8_t port) {
    // dirty hack: the last byte of the mac address is the port number. this makes it easy to filter out specific ports via
    // wireshark
...
void fifo_init(fifo_t* fifo, int fifo_count, int block_size) {
    ...
    for (i = 0; i < fifo_count; ++i) {
        ...
        for (j = 0; j < block_size; ++j) { fifo[i].pbl[j].len = 0; }
    }
}
...
void fifo_create_select_set(fifo_t* fifo, int fifo_count, fd_set* fifo_set, int* max_fifo_fd) {
    ...
    for (i = 0; i < fifo_count; ++i) {
        ...
        if (fifo[i].fd > *max_fifo_fd) { *max_fifo_fd = fifo[i].fd; }
    }
}

void pb_transmit_packet(pcap_t* ppcap, int seq_nr, uint8_t* packet_transmit_buffer, int packet_header_len,
                        const uint8_t* packet_data, int packet_length) {
    int   rtry_left = 8;
    ...
    while (1) {
        ...
        if (r != plen) {
            ...
            TIME_SAMPLE(t_err);
            do { TIME_SAMPLE(t_now); } while (TIME_DIFF(t_now, t_err) < delay);
...
void pb_transmit_block(packet_buffer_t* pbl, pcap_t* ppcap, int* seq_nr, int port, int packet_length,
                       uint8_t* packet_transmit_buffer, int packet_header_len, int data_packets_per_block,
...
int main(int argc, char* argv[]) {
    ...
    while (!fBrokenSocket) {
        ...
        for (i = 0; i < param_fifo_count && ret; ++i) {
            if (!FD_ISSET(fifo[i].fd, &rdfs)) { continue; }
            ...
            if (pb->len >= param_min_packet_length) {
                ...
                ph->data_length = pb->len - sizeof(payload_header_t); // write the length into the packet. this is needed since
                                                                      // with fec we cannot use the wifi packet lentgh anymore. We
                                                                      // could also set the user payload to a fixed size but this
                                                                      // would introduce additional latency since tx would need to
                                                                      // wait until that amount of data has been received
                pcnt++;

                // check if this block is finished
                if (fifo[i].curr_pb == param_data_packets_per_block - 1) {
                    pb_transmit_block(fifo[i].pbl, ppcap, &(fifo[i].seq_nr), i + param_port, param_packet_length,
        ...
        if (report_due && delta_report > 1) {
            ...
            printf("t:%.1f rtry:%d pkts:%d,byt:%d CUR:byt/s:%.1f,mbit/s:%.1f AVG:byt/s:%.1f,mbit/s:%.1f\n", delta_start,
                   g_retries, g_pkts_sent_total, g_bytes_sent_total, bytes_per_sec_inst, mbit_per_sec_inst, bytes_per_sec_avg,
                   mbit_per_sec_avg);
