...
int main(int argc, char* argv[]) {
    ...
    unsigned char headers[] = {
        ...
        // 0x18, 0x00, // 0x18 is b4,b3 is F_FCS (fcs will removed and recalculated), F_FRAG (frame will be fragmented if beyond
        // frag thr
    ...
    pcap = pcap_open_live("wlan1" /*device */, 0 /* snapshot length */, 0 /* promisc */, 0 /* read timeout in miliseconds */,
    ...
    while (1) {
        ...
        pkt_idx += 1;
        if (pkt_idx < 100 || 0 == (pkt_idx % 100)) { printf("injected packet %d\n", pkt_idx); }
    }
